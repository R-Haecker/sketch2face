...
class Iterator(TemplateIterator):
    def __init__(self, config, root, model, *args, **kwargs):
        ...
        self.optimizer_G = torch.optim.Adam(itertools.chain(self.model.netG.parameters()), lr=self.config["learning_rate"], betas=(.5, .999)) # betas=(opt.beta1, 0.999))
        D_lr_factor = self.config["optimization"]["D_lr_factor"] if "D_lr_factor" in config["optimization"] else 1
        self.optimizer_D = torch.optim.Adam(self.model.netD.parameters(), lr= D_lr_factor * self.config["learning_rate"], betas=(.5, .999)) # betas=(opt.beta1, 0.999))
