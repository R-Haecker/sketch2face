...
class Iterator(TemplateIterator):
    def __init__(self, config, root, model, *args, **kwargs):
        ...
        assert self.config["model"] == "model.wgan_gradient_penalty.WGAN_GP", "This iterator does only support the model: wgan_gradient_penalty.WGAN_GP"
        ...
        if self.config["model"] == "model.cycle_gan2.CycleGAN_Model":
            self.vae = True
        elif :
            self.vae = False
        else:
            assert 1==0, "Wrong Iterator for model. This iterator works only with WGAN_GP or CycleGAN_2"
    ...
    def step_op(self, model, **kwargs):
        ...
        images = kwargs["image_{}".format(self.config["model_type"])]
        ...
        def log_op():
            logs = self.prepare_logs(losses, images, fake_images)
            return logs
    ...
    def prepare_logs(self, losses, input_images,fake_images):
    ...
    def calculate_gradient_penalty(self, real_images, fake_images):
        ...
        self.logger.debug("fake_images.shape: " + str(fake_images.shape))
